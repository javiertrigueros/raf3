...
class Taggable extends Doctrine_Template
{
    ...
    /**
     * devuelve un array de ids de tags
     * 
     * @param <conn $conn>
     * @return <array of int>
     */
    public function getTagsArrayIds($conn = false)
    {
        $classModelRelated = get_class($this->getInvoker());
        $objectRelatedId = $this->getInvoker()->id;
        
        return Doctrine::getTable('Tag')->getTagsArrayIds($classModelRelated, $objectRelatedId, $conn);
    }
    
    public function getTagObjects()
    {
      
       return Doctrine::getTable('Tag')->getTags(
    ...
    public function saveTags($tagArray, Doctrine_Connection $conn)
    {
      
      if ($tagArray 
         && (is_array($tagArray)) && (count($tagArray) > 0))
      {
      
        $currentSavedTags = $this->getSavedTags();
        $removeTags = array();

        $newTags =  Doctrine_Core::getTable('Tag')->getTagNamesByArray($tagArray);
        
        $classModelRelated = get_class($this->getInvoker());
        $objectRelatedId = $this->getInvoker()->id;
            
        if ($currentSavedTags 
            && is_array($currentSavedTags) 
            && (count($currentSavedTags) > 0))
        {
            
            if ($newTags && !is_array($newTags))
            {
              $newTags = array($newTags);
            }
            
            $removeTags = array_diff($currentSavedTags, $newTags);
            
            $this->set_saved_tags($this->getInvoker(), array());
            $this->set_tags($this->getInvoker(), $newTags);
            $this->set_removed_tags($this->getInvoker(), $removeTags);
        }
        else
        { 
            if ($newTags && !is_array($newTags))
            {
             ...
             $this->set_saved_tags($this->getInvoker(), array());
             $this->set_tags($this->getInvoker(),  $newTags);
             $this->set_removed_tags($this->getInvoker(), array());
        }
        ...
        foreach ($tagArray as $tagId)
        {
            ...
            $tagging->setTaggableId($objectRelatedId);
            $tagging->setTaggableModel($classModelRelated);
            $tagging->save($conn);
        }  
      }
      else
