<?php
  ...
  $currentPage = isset($currentPage) ? $sf_data->getRaw('currentPage') : false;
  
  $editable = isset($editable) ? $sf_data->getRaw('editable') : false;
  
  $showTitle = isset($showTitle) ? $sf_data->getRaw('showTitle') : false;
  
  $realSlug = isset($realSlug) ? $sf_data->getRaw('realSlug') : '';
?>
<?php use_helper('a','ar') ?>

<?php slot("a-slot-controls-$pageid-$name-$permid") ?>
     <?php include_partial('arMenuSecundarySlot/choose', array('form' => $form, 'formMenu' => $formMenu,  'class' => 'a-btn icon a-media', 'id' => $id, 'options' => $options, 'name' => $name, 'slug' => $slug, 'permid' => $permid)) ?>
...
<?php if ($rootNode && $editable): ?>
   <?php include_js_call('arMenuSecundarySlot/jsEnableEditControl', array('id' => $id)) ?>
<?php endif ?>

        
<?php include_partial("arMenuSecundarySlot/menuList", 
        array('class' => 'nav secondary-menu',
              'slug' => $realSlug,
              'rootNode' =>  $rootNode,
