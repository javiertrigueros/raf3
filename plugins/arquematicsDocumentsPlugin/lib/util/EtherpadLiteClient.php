  ...
  */
  static public function getInstance()
  {
     if (self::$instancia == null) {
          ...
          self::$instancia = new EtherpadLiteClient(
                  sfConfig::get('app_docs_config_etherpad_key'),
                  sfConfig::get('app_docs_config_etherpad_api_url')
                  );
     }
  ...
  /**
   * genera un grupo con una id unica
   * 
   * @return groupId
   */
  public function createGroupAuto()
  {
     $this->generatedDocId = $this->generateDocId();
     return $this->createGroupIfNotExistsFor($this->generatedDocId);
  }

  // this functions helps you to map your application group ids to etherpad lite group ids 
  public function createGroupIfNotExistsFor($groupMapper)
  {
    $mappedGroup = $this->post("createGroupIfNotExistsFor", array(
   ...
   * @return type
   */
  public function createGroupPad($groupID, $padName = false, $text = '')
  {
      $newPad = $this->post("createGroupPad", array(
                    "groupID" => $groupID,
                    "padName" => (!$padName)?$this->generatedDocId:$padName,
                    "text"    => $text
                ));
  ...
  public function createAuthor($name){
    $author = $this->post("createAuthor", array(
      "name" => $name
    ));
  ...
  public function createAuthorIfNotExistsFor($authorMapper, $name){
    $author = $this->post("createAuthorIfNotExistsFor", array(
  ...
  public function createSession($groupID, $authorID, $validUntil){
    $session = $this->post("createSession", array(
      "groupID"    => $groupID,
      "authorID"   => $authorID,
   ...
   * @param type $padID
   * @param type $rev
   * @return <string> contenido en un string JSON
   */
  public function getJSON($padID, $rev = null){
    $params = array("padID" => $padID);
    if (isset($rev)){
      $params["rev"] = $rev;
    }
    return json_encode($this->get("getJSON", $params));
  }
   ...
   * @return type
   */
  public function setJSON($padID, $json){
    return $this->post("setJSON", array(
      "padID" => $padID, 
      "json"  => $json
    ));
  }
  ...
  public function getRevisionsCount($padID){
    $ret = $this->get("getRevisionsCount", array(
      "padID" => $padID
    ));
  ...
  public function getReadOnlyID($padID){
      ...
      $ret = $this->get("getReadOnlyID", array(
                "padID" => $padID
            ));
      
      return $ret['readOnlyID'];
  ...
  public function getTitle($padID)
  {
    return $this->get("getTitle", array(
      "padID" => $padID
        ));
  }
  
  public function getSavedRevisions($padID)
  {
     return $this->get("getSavedRevisions", array(
      "padID" => $padID
        )); 
  }
     ...
     * @return <string> 
     */
   private static function generateDocId()
   {
        return uniqid(substr(time().rand(11111, 99999), 0, 20 ), true);
   }
}
