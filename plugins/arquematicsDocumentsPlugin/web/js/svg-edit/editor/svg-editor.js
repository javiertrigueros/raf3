...
(function() {
	...
	window.svgEditor = (function($) {
		...
		editor.storagePromptClosed = false; // For use with ext-storage.js
                ...
                //boton de layer pulsado
                editor.layerOpen = false;
		...
		var svgCanvas, urldata,
			...
			*/
			...
			defaultConfig = {
				...
				imgPath: '/arquematicsDocumentsPlugin/js/svg-edit/editor/images/',
				langPath: '/arquematicsDocumentsPlugin/js/svg-edit/editor/locale/',
				extPath: '/arquematicsDocumentsPlugin/js/svg-edit/editor/extensions/',
				jGraduatePath: '/arquematicsDocumentsPlugin/js/svg-edit/editor/jgraduate/images/',
				// DOCUMENT PROPERTIES
		...
		*/
		...
		editor.init = function () {
			...
			var extFunc = function() {
				...
				editor.putLocale(editor.lang, good_langs);
			};
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				placement: {
					...
					'#tool_docprops_save,#tool_prefs_save': 'ok',
					'#tool_docprops_cancel,#tool_prefs_cancel': 'cancel',
			...
			var populateLayers = function() {
				...
				while (layer--) {
					var name = drawing.getLayerName(layer),
                                            layerVisibility = !drawing.getLayerVisibility(name),
                                            $layerTr = $('<tr class="layer">').toggleClass('layersel', name === currentLayerName),
                                            $layerVis = $('<td class="layervis">').toggleClass('layerinvis', layerVisibility),
                                            $layerName = $('<td class="layername">' + name + '</td>');
                                        
                                        if (!layerVisibility)
                                        {
                                          $layerVis.append('<span class="glyphicon glyphicon-eye-open"></span>');  
                                        }
                                        else
                                        {
                                          $layerVis.append('<span class="glyphicon glyphicon-eye-close"></span>');        
                                        }
				...
				while (num-- > 0) {
					...
					layerlist.append('<tr><td style="color:white">_</td><td/></tr>');
                        ...
                        var hideSourceEditor = function() {
                                $('#svg-source-editor').modal({show: false});
                                
				editingsource = false;
			};
                        
                        $('#source-cancel-extra,#source_cancel').bind( "click", function( e ){
                            hideSourceEditor();
                        } ); 
			...
			var showSourceEditor = function(e, forSaving) {
				...
				origSource = svgCanvas.getSvgString();
                                ...
                                $('#svg-source-editor').modal({show: true});
                        ...
                        var saveHandler = function(wind, svg) {
                            ...
                            var dataImage = svg,
                                ...
                                callBack = function (formData) {
                                    $.ajax({
                                        type: (svgEditor.autoload)?'PUT':"POST",
                            ...
                            if (titleText.length > 0)
                            {
                                ...
                                dataImage = arquematics.codec.encodeURIData(dataImage);

                                if (arquematics.crypt)
                                {
                                    var pass = (!editor.PASS)?
                                            //genera un pass aleatorio
                                            arquematics.utils.randomKeyString(50):
                                            //pone el mismo pass
                                            editor.PASS
                                    , data = {
                                        //en el editor nunca se comparte
                                        "note[share]"            :0,
                                        "note[task_complete]"    :$('#note_task_complete').val(),
                                        "note[task_all]"         :$('#note_task_all').val(),
                                        "note[trash]"            :$('#note_trash').val(),
                                        "note[is_favorite]"      :$('#note_is_favorite').val(),
                                        "note[pass]"             :pass,
                                        "note[title]"            :arquematics.simpleCrypt.encryptBase64(pass ,$('#note_title').val()),
                                        "note[_csrf_token]"      :$('#note__csrf_token').val(),
                                        "note[type]"             :editor.DIAGRAM_TYPE,
                                        "note[data_image]"       :arquematics.simpleCrypt.encryptBase64(pass , 'data:image/svg+xml;charset=utf-8,' + dataImage),
                                ...
                                {
                                    $('#note_data_image').val( 'data:image/svg+xml;charset=utf-8,' + dataImage);
                                    $('#note_content').val(dataJson);
                                    $('#note_type').val(editor.DIAGRAM_TYPE);
                                    //en el editor nunca se comparte
                                    $('#note_share').val(0);
			...
			var toolButtonClick = editor.toolButtonClick = function(button, noHiding) {
				...
				$('#styleoverrides').text('');
                                ...
                                //cursor
                                workarea.removeClass('zoom-max')
                                        .removeClass('zoom-min')
                                        .removeClass('tool-panning');
                                
				workarea.css('cursor', 'auto');
                                
                                $('.tool_button_current').each(
                                   function (){
                                        var $node = $(this);
                                        if ($node.attr('id') !== 'tool_layer')
                                        {
                                            $node.removeClass('tool_button_current');
                                        }
                                   }
                                );
			...
			var updateContextPanel = function() {
				...
				$('#right-panel, #sidepanels, #selected_panel, #multiselected_panel, #g_panel, #rect_panel, #circle_panel,'+
...
//					}
					...
					if (!is_node && currentMode != 'pathedit') {
						$('#sidepanels, #selected_panel').show();
						...
						$('#tool_add_subpath').removeClass('push_button_pressed');
...
//					}
					...
					if (el_name === 'a') {
						...
						$('#sidepanels,#g_panel').show();
					}

					if (elem.parentNode.tagName === 'a') {
						if (!$(elem).siblings().length) {
							$('#sidepanels,#a_panel').show();
					...
					if (panels[el_name]) {
						...
						$('#sidepanels,#' + el_name + '_panel').show();
						...
						if (el_name == 'text') {
							$('#text_panel').css('display', 'block');
							if (svgCanvas.getItalic()) {
								$('#tool_italic').addClass('push_button_pressed');
							} else {
								$('#tool_italic').removeClass('push_button_pressed');
							}
							if (svgCanvas.getBold()) {
								$('#tool_bold').addClass('push_button_pressed');
							} else {
								$('#tool_bold').removeClass('push_button_pressed');
							}
							...
							$('#sidepanels,#container_panel').show();
					...
					$('#sidepanels,#multiselected_panel').show();
					menu_items
...
//				var elems = $('.tool_button, .push_button, .tool_button_current, .disabled, .icon_label, #url_notice, #tool_open');
				var sel_toscale = '#tools_top .toolset, #editor_panel > *, #sa > *,'+
...
//					}
			...
			var addAltDropDown = function(elem, list, callback, opts) {
				var button = $(elem).find('.cmd-button-dropdown'),
                                    list = $(list);
				...
				$(window).mouseup(function(evt) {
                                        ...
                                        {
                                            list.css({top:position.top + list.height() + 25, left:position.left  + list.width()});
                                        }
					...
					$.each(ext.buttons, function(i, btn) {
						...
						if (!svgicons) {
							...
							svgicon = btn.svgicon || btn.id;
								...
								placement_obj['#' + id] = svgicon;
                                                        ...
                                                        {
                                                           //placement_obj['#' + id ] = false;
                                                           //fallback_obj[id] = false;  
                                                        }
						}
                                                */
						...
						var button = $((btn.list || btn.type === 'app_menu') ? '<li/>' : '<div/>')
							...
							.addClass(cls);
                                                ...
                                                if (!btn.svgicon)
                                                {
                                                   button.append($(btn.icon));     
                                                }
                                                
						if (!btn.includeWith && !btn.list) {
							if (('position' in btn) && (btn.type !== 'app_menu'))
                                                        {
...
//					$('.contextMenu').hide();
			...
			editor.addDropDown = function(elem, callback, dropUp, $focusControl, special) {
				if ($(elem).length == 0) {return;} // Quit if called on non-existant element
				var button = $(elem).find('.cmd-button-dropdown'),
                                    $focusControl = $focusControl || false,
				...
				button.bind('mousedown',function() {
					if (!button.hasClass('down')) {
                                                ...
                                                //esta activo el boton layer
                                                if (editor.layerOpen)
                                                {
                                                  pos.left -= $('#sidr-menu-layers').outerWidth(true);       
                                                }
			...
			editor.addDropDown('#blur_dropdown', $.noop, false, $('#blur'), true);
			...
			*/
					...
					svgCanvas.setMode('line');
                         ...
                         $(document).ready(function() {
                             $('#tool_layer').sidr({
                                ...
                                body: 'body',
                                renaming: false,
                                absIds: '#tools_bottom, #ruler_corner, #ruler_x, #workarea',
                                source: '#layerpanel'
                            }); 
                        });
                        ...
                        var clickLayer = function() {
                                    //pequeño arreglo
                                    $('#ruler_y').height($('#workarea').height() - 30);
                                    
                                    if ($('#tool_layer').hasClass('tool_button_current'))
                                    {
                                       $('#tool_layer').addClass('tool_button');
                                       $('#tool_layer').removeClass('tool_button_current');  
                                    }
                                    else
                                    {
                                       $('#tool_layer').addClass('tool_button_current');
                                       $('#tool_layer').removeClass('tool_button');   
                                    }
                                    
                                    editor.layerOpen = $('#tool_layer').hasClass('tool_button_current');
			...
			var clickZoom = function() {
				...
				if (toolButtonClick('#tool_zoom')) {
                                        //console.trace();
                                        //svgCanvas.current_zoom = 1;
					svgCanvas.setMode('zoom');
                                        

                                        workarea.addClass('zoom-max');
					//workarea.css('cursor', zoomInIcon);
				}
			};
                        
                        var clickZoomMinus = function() {
				if (toolButtonClick('#tool_zoom_minus')) {
                                        //$('#zoom').val(multiplier * 100);
                                        //svgCanvas.current_zoom = 0.5;
					svgCanvas.setMode('zoomLess');
                                        
                                        workarea.addClass('zoom-min');
					//workarea.css('cursor', zoomInIcon);
				}
			};
			...
			var addSubPath = function() {
				...
				button.toggleClass('push_button_pressed');
				path.addSubPath(sp);
			};
...
//				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
				...
				$('#color_picker')
					...
					.css(curConfig.colorPickerCSS || {'left': pos.left, 'bottom': 70})
					.jGraduate(
					{
						paint: paint,
...
'					<defs><linearGradient id="gradbox_"/></defs></svg>', 'text/xml');
			...
			$('#tool_move_top').mousedown(function(evt) {
				$('#sidepanels,#tools_stacking').show();
				evt.preventDefault();
			});
		...
		//	}
			...
			Actions = (function() {
				...
				var tool_buttons = [
					...
					{sel: '#tool_layer', fn: clickLayer, evt: 'click', key: ['O', true]},
					...
					{sel: '#tool_zoom', fn: clickZoom, evt: 'mouseup', key: ['Z', true]},
                                        {sel: '#tool_zoom_minus', fn: clickZoomMinus, evt: 'mouseup', key: ['A', true]},
					{sel: '#tool_clear', fn: clickClear, evt: 'mouseup', key: ['N', true]},
					{sel: '#tool_save', fn: saveToServer, evt: 'mouseup', key: ['S', true]},
					{sel: '#tool_export', fn: clickExport, evt: 'mouseup'},
					{sel: '#tool_open', fn: clickOpen, evt: 'mouseup', key: ['O', true]},
					{sel: '#tool_import', fn: clickImport, evt: 'mouseup'},
					{sel: '#tool_source', fn: showSourceEditor, evt: 'click', key: ['U', true]},
					{sel: '#tool_wireframe', fn: clickWireframe, evt: 'click', key: ['F', true]},
					{sel: '.overlay,#tool_docprops_cancel,#tool_prefs_cancel', fn: cancelOverlays, evt: 'click', key: ['esc', false, false], hidekey: true},
					{sel: '#source_save', fn: saveSourceEditor, evt: 'click'},
...
//			});
		...
		editor.runCallbacks = function () {
			...
			isReady = true;
                 ...
                 */
                editor.arLoadDataFromString = function (){
                	console.log('entra aqui');
                    editor.ready(function() {
                        if (editor.autoload 
                            && arquematics.crypt)
                        {
                            editor.PASS = arquematics.crypt.decryptHexToString(editor.PASS);
                            editor.TITLE = arquematics.simpleCrypt.decryptBase64(editor.PASS, editor.TITLE);
                            editor.DATA = arquematics.simpleCrypt.decryptBase64(editor.PASS, editor.DATA);
                            
                            editor.DATA = arquematics.codec.decodeURIData(editor.DATA);
                            
                            $('#note_title').val(editor.TITLE);

                            loadSvgString(editor.DATA);
                        }
                        else if (editor.autoload)
                        {
                           loadSvgString(arquematics.codec.Base64.toString(editor.DATA, true));     
                        }
