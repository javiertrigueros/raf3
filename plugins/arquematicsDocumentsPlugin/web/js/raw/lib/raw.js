...
!function (exports, moment, arquematics){
  var raw = {
    //datos y parametros del diagrama
    content: {
        text: '',
        chart: '',
        metadata: [],
...
raw.multiFormat = function multiFormat(date) {
...
raw.parseDate = function parseDate(value, dataFormat){
       if (!dataFormat)
       {
        return value;
       }
  ...
  raw.parser = function (delimiter) {
    ...
    parser.metadata = function(string){
      raw.content.metadata = d3.entries(sniff(parser(string))).map(function (d){
  ...
  var model_dimension = function (id) {
    ...
    dimension.getId = function(_) {
      if (!arguments.length) return dimensionId;
      dimensionId = _ + "";
      return dimension;
    }
    ...
    dimension.type = function() {
      if (!dimension.value[0] || !dimension.value[0].type)
      {
         return;     
      }
      else
  ...
  raw.model = function () { 
    ...
    model.dimension = function(id) {
      id = id || dimensions.values().length;
      var dimension = model_dimension(id);
    ...
    model.instruction = function() {
      return dimensions.values()
        .filter(function(d){ return d.required() > d.value.length; })
        .map(function(d){ 
            var v = d.required() - d.value.length > 1 ?
    ...
    model.instruction = function() {
      return dimensions.values()
        ...
        .map(function(d){ 
            return {
               title: d.title(),
               more: d.required() - d.value.length,
               plural: d.required() - d.value.length > 1
            }
        });
    }
    /**
     * dimensiones del gráfico ya mapeadas
     * 
     *@return array
     */
    model.dimensionData = function() {
      return dimensions.values()
        //.filter(function(d){ return d.required() > d.value.length; })
        .map(function(d){ 
            return d.value
    ...
    model.setDimensionData = function(dataValues) {
      var index = 0;
      return dimensions.values()
        .map(function(d){ 
            d.value = dataValues[index];
            index++;
        });
    }
  ...
  raw.chart = function (id) { 
     ...
     id = id || raw.charts.values().length;
    ...
    /*
    chart.getText = function(key)
    {
        var ret = "";
        
        for (var index = 0, find = false; ((!find) && (index < langKeys.length)); index++)
        {
          find = (langKeys[index].key === key); 
          if (find)
          {
            ret =  langKeys[index].value; 
          }
        }
    ...
    chart.nameFunc = function(_) {
      if (!arguments.length) return nameFunc;
      nameFunc = _.toString();
      return chart;
    }
    ...
    // Options
    chart.stringOption = function(id){
      id = id || options.size();
  ...
  raw.findDataFormat = function(data, key){ 
        var ret = false
                , strValue = '';
                
                if (data.length > 0)
                {
                  for (var index = 0, find = false; ((!find) && (index < data.length)); index++)
                  {
                      strValue = $.trim(data[index][key]);
                      if (strValue.length > 0)
                      {
                        //EN
                        if (moment(strValue,'YYYY-MM-DD', true).isValid())
                        {
                            ret = 'YYYY-MM-DD';
                            find = true;
                        }
                        else if (moment(strValue,'YYYY-MM-DD HH:mm', true).isValid())
                        {
                            ret = 'YYYY-MM-DD HH:mm';
                            find = true;    
                        }
                        else if (moment(strValue,'YYYY-MM-DD HH:mm Z', true).isValid())
                        {
                            ret = 'YYYY-MM-DD HH:mm Z';
                            find = true;  
                        }
                        else if (moment(strValue,'YYYY/MM/DD', true).isValid())
                        {
                            ret = 'YYYY/MM/DD';
                            find = true;
                        }
                        ...
                        //ES
                        if (moment(strValue,'DD-MM-YYYY', true).isValid())
                        {
                            ret = 'DD-MM-YYYY';
                            find = true;
                        }
                        else if (moment(strValue,'DD-MM-YYYY HH:mm', true).isValid())
                        {
                            ret = 'DD-MM-YYYY HH:mm';
                            find = true;    
                        }
                        else if (moment(strValue,'DD-MM-YYYY HH:mm Z', true).isValid())
                        {
                            ret = 'DD-MM-YYYY HH:mm Z';
                            find = true;  
                        }
                        else if (moment(strValue,'DD/MM/YYYY', true).isValid())
                        {
                            ret = 'DD/MM/YYYY';
                            find = true;
                        }
                        ...
                        //EN 2014-9-30
                        else if (moment(strValue,'YYYY-M-D', true).isValid())
                        {
                            ret = 'YYYY-M-D';
                            find = true;
                        }
                        else if (moment(strValue,'YYYY-M-D HH:mm', true).isValid())
                        {
                            ret = 'YYYY-M-D HH:mm';
                            find = true;    
                        }
                        else if (moment(strValue,'YYYY-M-D HH:mm Z', true).isValid())
                        {
                            ret = 'YYYY-M-D HH:mm Z';
                            find = true;  
                        }
                        else if (moment(strValue,'YYYY/M/D', true).isValid())
                        {
                        ...
                        //ES 30-9-2014
                        else if (moment(strValue,'D-M-YYYY', true).isValid())
                        {
                            ret = 'D-M-YYYY';
                            find = true;
                        }
                        else if (moment(strValue,'D-M-YYYY HH:mm', true).isValid())
                        {
                            ret = 'D-M-YYYY HH:mm';
                            find = true;    
                        }
                        else if (moment(strValue,'D-M-YYYY HH:mm Z', true).isValid())
                        {
                            ret = 'D-M-YYYY HH:mm Z';
                            find = true;  
                        }
                        else if (moment(strValue,'D/M/YYYY', true).isValid())
                        {
  ...
  raw.isDate = function(value){ 
    ...
    return moment(value,["YYYY-MM-DD", "YYYY-MM-DD HH:mm","YYYY-MM-DD HH:mm Z"], true).isValid()
           || moment(value,["YYYY/MM/DD", "YYYY/MM/DD HH:mm","YYYY/MM/DD HH:mm Z"], true).isValid()
           //ES
           || moment(value,["DD-MM-YYYY", "DD-MM-YYYY HH:mm","DD-MM-YYYY HH:mm Z"], true).isValid()
           || moment(value,["DD/MM/YYYY", "DD/MM/YYYY HH:mm","DD/MM/YYYY HH:mm Z"], true).isValid()
           //EN   
           ||  moment(value,["YYYY-M-D", "YYYY-M-D HH:mm","YYYY-M-D HH:mm Z"], true).isValid()
           || moment(value,["YYYY/M/D", "YYYY/M/D HH:mm","YYYY/M/D HH:mm Z"], true).isValid()
           //ES
           || moment(value,["D-M-YYYY", "D-M-YYYY HH:mm","D-M-YYYY HH:mm Z"], true).isValid()
           || moment(value,["D/M/YYYY", "D/M/YYYY HH:mm","D/M/YYYY HH:mm Z"], true).isValid()
       ;
  }
...
}(typeof exports !== 'undefined' && exports || this, moment, arquematics);
