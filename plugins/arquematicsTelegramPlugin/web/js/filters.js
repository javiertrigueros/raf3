...
angular.module('myApp.filters', [])
  ...
  .filter('userStatus', ['$filter','gettextCatalog', function($filter,gettextCatalog) {
    return function (user) {
      ...
      return gettextCatalog.getString('last seen') + ' ' + $filter('amCalendar')(parseInt(user.status.was_online, 10) * 1000);
    };
  }])
  ...
  .filter('dateOrTime', ['$filter','gettextCatalog', function($filter,gettextCatalog) {
    var cachedDates = {};
    ...
    return function (timestamp) {
      ...
      if (diff > 518400000) { // 6 days
        format = 'shortDate';
        cachedDates[timestamp] = $filter('date')(ticks, format);
      }
      else if (diff > 43200000) { // 12 hours
        format = 'EEE'; 
        cachedDates[timestamp] = gettextCatalog.getString($filter('date')(ticks, format));
      }
      else
      {
         cachedDates[timestamp] = $filter('date')(ticks, format);    
