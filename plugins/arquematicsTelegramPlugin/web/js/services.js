...
.service('AppConfigManager', function ($q, $rootScope) {
  var testPrefix = $rootScope.userConfig.id + '_';
  var cache = {};
...
.service('AppUsersManager', function ($rootScope, $modal, $modalStack, MtpApiFileManager, MtpApiManager, RichTextProcessor, SearchIndexManager) {
  ...
  function getUserPhoto(id, placeholder) {
    ...
    if (id == 333000) {
      return {
        placeholder: '/arquematicsTelegramPlugin/images/placeholders/DialogListAvatarSystem@2x.png'
      }
    };

    return {
      placeholder: '/arquematicsTelegramPlugin/images/placeholders/' + placeholder + 'Avatar'+((Math.abs(id) % 8) + 1)+'@2x.png',
  ...
  function wrapForFull (id) {
    ...
    user.thumb = {
      placeholder: '/arquematicsTelegramPlugin/images/placeholders/UserAvatar'+((Math.abs(id) % 8) + 1)+'@2x.png',
      location: user && user.photo && user.photo.photo_small,
      width: 120,
      height: 120,
      size: 0
  ...
  function openUser (userID, accessHash) {
    ...
    var modalInstance = $modal.open({
      templateUrl: '/arquematicsTelegramPlugin/js/partials/user_modal.html',
...
.service('AppChatsManager', function ($rootScope, $modal, MtpApiFileManager, MtpApiManager, AppUsersManager, RichTextProcessor) {
  ...
  function getChatPhoto(id, placeholder) {
    ...
    return {
      placeholder: '/arquematicsTelegramPlugin/images/placeholders/' + placeholder + 'Avatar'+((Math.abs(id) % 4) + 1)+'@2x.png',
  ...
  function wrapForFull (id, fullChat) {
    ...
    chatFull.thumb = {
      placeholder: '/arquematicsTelegramPlugin/images/placeholders/GroupAvatar'+((Math.abs(id) % 4) + 1)+'@2x.png',
      location: chat && chat.photo && chat.photo.photo_small,
      width: 120,
      height: 120,
      size: 0
  ...
  function openChat (chatID, accessHash) {
    ...
    var modalInstance = $modal.open({
      templateUrl: '/arquematicsTelegramPlugin/js/partials/chat_modal.html',
...
.service('AppPhotosManager', function ($modal, $window, $rootScope, MtpApiFileManager, AppUsersManager) {
  ...
  function wrapForHistory (photoID) {
    var photo = angular.copy(photos[photoID]) || {_: 'photoEmpty'},
        ...
        thumb = {
          placeholder: '/arquematicsTelegramPlugin/images/placeholders/PhotoThumbConversation.gif',
          width: width,
          height: height
        };
  ...
  function wrapForFull (photoID) {
    var photo = wrapForHistory(photoID),
        ...
        full = {
          placeholder: '/arquematicsTelegramPlugin/images/placeholders/PhotoThumbModal.gif',
          width: fullWidth,
          height: fullHeight
        };
  ...
  function openPhoto (photoID, accessHash) {
    ...
    var modalInstance = $modal.open({
      templateUrl: '/arquematicsTelegramPlugin/js/partials/photo_modal.html',
...
.service('AppVideoManager', function ($rootScope, $modal, $window, MtpApiFileManager, AppUsersManager) {
  ...
  function wrapForHistory (videoID) {
    var video = angular.copy(videos[videoID]),
        ...
        thumb = {
          placeholder: '/arquematicsTelegramPlugin/images/placeholders/VideoThumbConversation.gif',
          width: width,
          height: height
        };
  ...
  function wrapForFull (videoID) {
    var video = wrapForHistory(videoID),
        ...
        fullPhotoSize = video,
        full = {
          placeholder: '/arquematicsTelegramPlugin/images/placeholders/VideoThumbModal.gif',
          width: fullWidth,
          height: fullHeight,
        };
  ...
  function openVideo (videoID, accessHash) {
    ...
    var modalInstance = $modal.open({
      templateUrl: '/arquematicsTelegramPlugin/js/partials/video_modal.html',
...
.service('AppDocsManager', function ($rootScope, $modal, $window, $timeout, MtpApiFileManager) {
  ...
  function wrapForHistory (docID) {
    ...
    var doc = angular.copy(docs[docID]),
        ...
        thumb = {
          placeholder: '/arquematicsTelegramPlugin/images/placeholders/DocThumbConversation.jpg',
          width: width,
          height: height
        };
  ...
  function wrapRichText(text, options) {
    ...
    if (emojiFound) {
      text = text.replace(/<span class="emoji emoji-file-([0-9a-f]+?)"(.+?)<\/span>/g,
                          '<span class="emoji" style="background-image: url(\'/arquematicsTelegramPlugin/js/vendor/gemoji/images/$1.png\')"$2</span>');
    }
...
.service('NotificationsManager', function ($rootScope, $window, $timeout, $interval, $q, MtpApiManager, AppPeersManager, IdleManager, AppConfigManager) {
  ...
  function playSound () {
    var filename = '/arquematicsTelegramPlugin/images/sound_a.wav';
...
.service('ErrorService', function ($rootScope, $modal) {
  ...
  function showSimpleError (title, description) {
    return showError ('/arquematicsTelegramPlugin/js/partials/error_modal.html', {
      title: title,
