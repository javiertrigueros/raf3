    ...
    return {
      ...
      compile: function (element, attrs, transclude) {
        return function ($scope, $element) {
          ...
          assert(!attrs.translateJson || attrs.translateCount, 'translate-json', 'translate');
          
          var countFn = $parse(attrs.translateN || attrs.translateCount);
          ...
          transclude($scope, function (clone) {
            ...
            return $scope.$watch(function () {
              ...
              var translated = '';
              if (attrs.translateJson)
              {
                  try{
                     var data = eval(attrs.translateJson),
                     count = countFn($scope);
                     
                     if (count >= data.length)
                     {
                       translated = gettextCatalog.getString(data[data.length -1]);      
                     }
                     else
                     {
                       translated = gettextCatalog.getString(data[count]);       
                     }
                     
                     translated = $interpolate(translated)({count:count});
                  }
                  catch(err)
                  {
                    //Handle errors here
