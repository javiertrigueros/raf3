...
<ul class="a-ui">
	...
	<?php if(isset($layout['fields']['categories'])): ?>
		<?php if (count($mediaItem->getCategories())): ?>
			<li class="a-media-item-categories a-media-item-meta"><?php echo __('Categories: %categories%', array('%categories%' => get_partial('aMedia/showCategories', array('categories' => $mediaItem->getCategories()))), 'apostrophe') ?></li>
		<?php endif ?>
	<?php endif ?>

	<?php if(isset($layout['fields']['tags'])): ?>
		<?php if (count($mediaItem->getTags())): ?>
			<li class="a-media-item-tags a-media-item-meta"><?php echo __('Tags: %tags%', array('%tags%' => get_partial('aMedia/showTags', array('tags' => $mediaItem->getTags()))), 'apostrophe') ?></li>
		<?php endif ?>
	<?php endif ?>
	...
	<?php if(isset($layout['fields']['downloadable'])): ?>
		<?php if ($mediaItem->getType() !== 'video'): ?>
			<li class="a-media-item-download a-media-item-meta">
				<?php echo link_to('<span class="icon"></span>'.__("Download Original", null, 'apostrophe'),	"aMediaBackend/original?" .http_build_query(array("slug" => $mediaItem->getSlug(), "format" => $mediaItem->getFormat())), array("class"=>"a-btn icon a-download lite alt")) ?>
			</li>
		<?php endif ?>
	<?php endif ?>
</ul>
