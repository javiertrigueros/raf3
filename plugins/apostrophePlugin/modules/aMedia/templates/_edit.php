...
<div class="a-form-row tags">
  <?php echo $form['tags']->renderLabel() ?>
	<div class="a-form-field">
  	...
  	<?php $options = array('tags-label' => __('Add',null,'apostrophe'), 'popular-tags' => $popularTags, 'tags-label' => ' ', 'commit-selector' => $submitSelector, 'typeahead-url' => a_url('taggableComplete', 'complete')) ?>
...
<?php if ((!$fileShown) && isset($form['file'])): ?>
    ...
    <?php if ($item && $item->getDownloadable()): ?>
			<?php echo link_to('<span class="icon"></span>'.__("Download Original", null, 'apostrophe'),	"aMediaBackend/original?" .http_build_query(array("slug" => $item->getSlug(), "format" => $item->getFormat())), array("class"=>"a-btn icon a-download lite alt")) ?>
		<?php endif ?>
	</div>
<?php endif ?>
<?php if ($single): ?>
...
<?php //a_js_call("$('#".$form['view_is_secure']->renderId()."').bootstrapSwitch(?);", array('descId' => '#' .$form['view_is_secure']->renderId())); ?>
