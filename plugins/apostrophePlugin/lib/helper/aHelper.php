...
function getFilesContent($type, $files)
{
    $content  = '';
...
function _a_get_assets_body($type, $assets, $urlMap = array())
{
  ...
  foreach ($sets as $optionsJson => $files)
  {
    ...
    if (!$assetStatCache->get($groupPathname))
    {
      ...
      {
        $content = getFilesContent($type,  $files);
        
        $opts['s3']=array( 'ContentType'=> ($type === 'stylesheets')?'text/css':'text/javascript' ); 
        
        //para arreglar typos mime erroneos en s3
        ...
        if (sfConfig::get('app_a_copy_assets_then_rename', true))
        {
          ...
          {
            file_put_contents($groupPathname . '.tmp', $content, FILE_APPEND,$context);
          }
        ...
        else
        {
          ...
          {
            file_put_contents($groupPathname, $content, FILE_APPEND,$context);
          }
        }
      }
...
/**
 * The a_tab helper returns a tab compatible with the markup used
 * by Apostrophe's standard navigation. This is useful when your site
 * is not dominated by Apostrophe pages and you wish to implement your
 * own tabs in the same style.
 *
 * This helper is similar to link_to in that the URL parameter
 * is passed through url_for, so @my_route is automatically
 * converted to a final URL for you.
 *
 * The following conveniences are implemented to streamline the
 * effort involved:
 *
 * The 'first' class is automatically set on the first tab that
 * is returned during this PHP request as a whole. You can override
 * this behavior by passing the 'first' option (true or false).
 *
 * The 'current' class is automatically set on the tab if the
 * URL generated by the $url parameter matches the current
 * request URL or appears at the start of the current request
 * URL, followed by a slash. You can override this behavior by
 * passing the 'current' option (true or false).
 *
 * Additional options:
 *
 * The 'class' option is supported and is applied
 * to the <li> tag for the tab.
 *
 * The 'open' option, if true, prevents </li> from being
 * added automatically. This is useful if you are implementing
 * dropdown menus via nested <ul>s.
 *
 ...
 *
 */
function a_tab($label, $url, $options = array())
{
  // Dare to use the global keyword to automatically mark
  // the first tab - but also allow an override via options
  // for anything we get sneaky with that way
  global $first;
  if (!isset($first)) 
  {
    $first = true;
  ...
  $uri = sfContext::getInstance()->getRequest()->getPathInfo();
  if (($url === $uri) || (substr($uri, 0, strlen($url) + 1) === ($url . '/')))
  {
    $current = true;
  }
  ...
  $link['href'] = $url;
  return '<li class="a-nav-item a-nav-item-0 ' . ($first ? 'first ' : '') . ($current ? 'a-current-page ' : '') . ' ' . $class . '">' . content_tag('a', $label, $link) . ($open ? '' : '</li>');
}