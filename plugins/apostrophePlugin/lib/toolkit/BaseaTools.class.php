...
class BaseaTools
{
  ...
  static public function getTemplates()
  {
    ...
    if (!Doctrine::getTable('aPage')
            ->hasEnginePage('aBlog'))
    {
      $templates['aBlog'] = array('default' => 'Blog');  
    }
    
    if (!Doctrine::getTable('aPage')
            ->hasEnginePage('aEvent'))
    {
      $templates['aEvent'] = array('default' => 'Events');  
    }
  ...
  static public function validatePageAccess(sfAction $action, $page)
  {
    ...
    if ((!$page) && sfConfig::get('sf_debug'))
    {
       $action->forward404Unless($page);
    }
    else if (!$page)
    {
      $action->forward(sfConfig::get('sf_error_404_module'), sfConfig::get('sf_error_404_action'));
    }
   ...
   * <@return string>
   */
  static public function getRealUrlPath()
  {
    $url = aTools::getRealUrl();
    $parseUrl = parse_url($url);
    
    return ($parseUrl && isset($parseUrl['path']))?$parseUrl['path']:'';
  }
...
$cdnCnv = str_replace(array('ñ', 'Ñ', 'ç', 'Ç'),
                      array('n', 'N', 'c', 'C'),
                      $cdnCnv);
...
$cdnCnv = str_replace(
  ...
  static protected $jsByName = array(
    ...
    'jquery' => '/apostrophePlugin/js/jquery-1.8.3.js',
    //'jquery' => '/apostrophePlugin/js/jquery-1.9.0.js',
    ...
    // :TODO mirar esto con más detalle. Javier 
    'jquery-ui' => '/apostrophePlugin/js/plugins/jquery-ui/ui/jquery-ui.js',
  ...
  /**
   * Lock names must be ascii alphanumeric and underscore characters
   * and must not be empty. If $wait is false, tries to get a lock
   * and returns false promptly if it is unable to do so right now,
   * true on success. If $wait is true, returns true on a successful
   * lock after waiting up to 30 seconds, otherwise throws an sfException.
   *
   * By default an implementation based on flock() is used, which is fine
   * for the vast majority of sites (see aLockFlock for details). If you
  ...
  /**
   * Release the most recent lock. It is not an error to call with no locks
   */
