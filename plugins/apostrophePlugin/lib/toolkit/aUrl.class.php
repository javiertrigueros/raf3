...
class aUrl
{
   ...
   * @return array
   */
  static public function convertUrlToArray($url) 
  {
        $ret = parse_url($url);
        
     
        if ($ret && is_array($ret) && (isset($ret['query'])))
        {
            $queryParts = explode('&', $ret['query']);
            if ($queryParts && is_array($queryParts))
            {
