...
class BaseaMediaActions extends aEngineActions
{
  ...
  public function executeSelected(sfWebRequest $request)
  {
    ...
    if (!aMediaTools::isMultiple())
    {
      ...
      {
        $after = aUrl::addParams($afterVal,
            array("aMediaId" => $newSelection[0]));
        
       /*
        
        $urlSata = aUrl::convertUrlToArray($after);
        
        if ($urlSata && is_array($urlSata) && isset($urlSata['slug']))
        {
          
           $urlSata['slug'] = urldecode($urlSata['slug']);
    ...
    {
      aMediaTools::clearSelecting();
      $url = aUrl::addParamsNoDelete($afterVal,
