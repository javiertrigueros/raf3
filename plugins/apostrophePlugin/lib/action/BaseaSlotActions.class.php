...
class BaseaSlotActions extends sfActions
{
  ...
  protected function setup($editing = false)
  {
    ...
    if ($this->hasRequestParameter('slug'))
    {
      ...
      /*
      $this->slug = urldecode($this->slug);
      $this->slug = preg_replace(
                   array('/a_event_show_id/','/a_blog_show_id/'),
                   array('ar_blog_event_edit','ar_blog_post_edit'),
                   $this->slug);*/
      
      //$this->slug = urldecode($this->getRequestParameter('slug'));
  ...
  protected function editSave($options = array())
  {
    ...
    $dataEvent =   array(
                ...
                'values' => $this->slot->value,
                'name' => $this->name);
    
    //print_r($dataEvent);
    $event = new sfEvent($this->page, 'ar.addslot', $dataEvent);
  ...
  protected function redirectToPage()
  {
    ...
    elseif ($this->hasRequestParameter('slug'))
    {
       //javier : Pequeño hack
       // hack
       $slug = preg_replace(
       ...
       return $this->redirect(urldecode($slug));
    }
    else
    {
