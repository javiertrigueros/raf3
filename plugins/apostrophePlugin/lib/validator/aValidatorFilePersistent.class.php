...
class aValidatorFilePersistent extends sfValidatorFile
{
  ...
  public function clean($value)
  {
    ...
    if ($newFile)
    {
      ...
      if ($persistid !== false)
      {
        ...
        //$persistentDirTmp = $this->getPersistentTmp();
        
        $filePath = "$persistentDir/$persistid.file";
       
        if (file_exists($cvalue['tmp_name']))
        {
          self::putFileInfo($persistid. '.extra', base64_encode(file_get_contents($cvalue['tmp_name'])));  
        }
        
        //intenta recuperar el archivo del cache si no existe
        if (!@copy($cvalue['tmp_name'], $filePath))
        {
           $fileData = self::getFileInfo($persistid.'.extra');
           file_put_contents($cvalue['tmp_name'], base64_decode($fileData));
    ...
    try
    {
      $event = new sfEvent($this, 'ar.fileAdded', array('item' => $data));
  ...
  static protected function getPersistentTmp()
  {
    return aFiles::getWritableDataFolder(array("tmp"));
  }
