...
class BaseaPageSettingsForm extends aPageForm
{
  ...
  { 
    ...
    else
    {
      if ($manage)
      {
        ...
        $this->setWidget('view_options', new arMenuOptionsWidget(
                array('choices' => $choices,
                    'expanded' => true,
                    'multiple' => false,
                    'default' => $default),
                array('class' => 'view-options-widget',
                      'data-default-choice' => $default)
                ));
        $this->setValidator('view_options', new sfValidatorChoice(array('choices' => array_keys($choices), 'required' => false)));
       ...
       $this->setWidget('view_individuals', new arPagePermissionsWidget(array('default' => $this->getEditIndividualsJSON(),
          'name-id' => 'individuals',
          'removeLabel' => a_('Remove'),
          'addLabel' => a_('+ Add User'),
          'extra' => false,
          'applyToSubpagesLabel' => a_('apply to subpages'),
          'hasSubpages' => false)));
        
        //$this->setWidget('view_individuals', new sfWidgetFormInputHidden(array('default' => $this->getViewIndividualsJSON())));
        
        $this->setValidator('view_individuals', new sfValidatorCallback(array('callback' => array($this, 'validateViewIndividuals'), 'required' => false)));
        
        $this->setWidget('view_groups', new arPagePermissionsWidget(array(
          'default' => $this->getEditGroupsJSON(),
          'name-id' => 'groups',
          'removeLabel' => a_('Remove'),
          'addLabel' => a_('+ Add Group'),
          'extra' => false,
          'applyToSubpagesLabel' => a_('apply to subpages'),
          'hasSubpages' => false)));
        ...
        $this->setValidator('view_groups', new sfValidatorCallback(array('callback' => array($this, 'validateViewGroups'), 'required' => false)));
      }
    }
    ...
    // approach in mind
    $this->setWidget('archived', new arMenuOptionsWidget(array(
      'expanded' => true,
      'choices' => array(false => "Published", true => "Unpublished"),
      'default' => ($this->getObject()->getArchived() == true)
    )));
    ...
    $options['tags-label'] = __('Add Tag', null, 'adminMenu');
    $options['popular-tags-label'] = a_('Popular Tags');
    ...
    if ($user->hasCredential('cms_admin'))
    {
      ...
      $this->setWidget('edit_individuals', new arPagePermissionsWidget(array('default' => $this->getEditIndividualsJSON(),
          'name-id' => 'individuals',
          'removeLabel' => a_('Remove'),
          'addLabel' => a_('+ Add User'),
          'extra' => false,
          'applyToSubpagesLabel' => a_('apply to subpages'),
          'hasSubpages' => false)));
      $this->setValidator('edit_individuals', new sfValidatorCallback(array('callback' => array($this, 'validateEditIndividuals'), 'required' => false)));
      //$this->setWidget('edit_groups', new sfWidgetFormInputHidden(array('default' => $this->getEditGroupsJSON())));
       
      $this->setWidget('edit_groups', new arPagePermissionsWidget(array(
          'default' => $this->getEditGroupsJSON(),
          'name-id' => 'groups',
          'removeLabel' => a_('Remove'),
          'addLabel' => a_('+ Add Group'),
          ...
          'applyToSubpagesLabel' => a_('apply to subpages'),
          'hasSubpages' => false)));
      $this->setValidator('edit_groups', new sfValidatorCallback(array('callback' => array($this, 'validateEditGroups'), 'required' => false)));
    }
    ...
    $this->setWidget('realtitle', new sfWidgetFormInputText(array('default' => aHtml::toPlaintext($this->getObject()->getTitle(), ENT_COMPAT, 'UTF-8')), array('placeholder' => __('Page title', null, 'configure'))));
    ...
    $this->setWidget('edit_admin_lock', new sfWidgetFormInputCheckbox() );
    $this->setValidator('edit_admin_lock', new sfValidatorBoolean(array('required' => false)));
   
    $this->setDefault('edit_admin_lock', $this->getObject()->isNew() || $this->getObject()->edit_admin_lock);
  ...
  protected function getViewGroupsJSON($forceParent = false)
  {
    ...
    $jinfos[] = array('id' => 'editors_and_guests', 'name' => a_('Editors and Guests'), 'selected' => $this->getObject()->view_guest, 'applyToSubpages' => false);
  ...
  public function updateObject($values = null)
  {
    ...
    if ((!isset($values['slug'])) && (isset($values['realtitle'])) && ($oldSlug !== '/'))
    {
      ...
      $newSlug = preg_replace('|/[^/]*$|', '/' . aTools::slugify( aTools::sanitizeText($values['realtitle']), false, false), $oldSlug);
      $suffix = '';
      $n = 0;
      while (true)
      {
