...
abstract class PluginaMediaItem extends BaseaMediaItem
{
  ...
  public function save(Doctrine_Connection $conn = null)
  {
    ...
    if ($new)
    {
      $event = new sfEvent($this, 'a.mediaAdded', array('item' => $this));
    }
    else
    {
      $event = new sfEvent($this, 'a.mediaEdited', array('item' => $this));
    }
    if (sfContext::hasInstance())
    {
  ...
  public function saveFile($file)
  {
    ...
    if (!file_exists($file))
    {
       ...
       $persistid = basename($file,'.file').'.extra';
       ...
       @file_put_contents($file, base64_decode($raw));
    }
  ...
  public function getScaled($options)
  {
    ...
    return $this->render(array_merge($params, array('cachedOnly' => true, 'authorize' => true)));
  }
  ...
  {    
    ...
    $aspectRatio = aMediaTools::getAspectRatio();// this returns 0 if aspect-width and aspect-height were not set
    if ($aspectRatio) 
    {
  ...
  //   $localUrl = $sf_relative_url_root . '/uploads/media_items/' . $basename;
    ...
    if (!$cached)
    {
      ...
      if (isset($options['authorize']) && $options['authorize'])
      {
         ...
         * hack
         * 
         * problemas con s3 para generar la imagen
         */
        /*
        if (sfConfig::get('app_a_static_url'))
        {    
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, sfContext::getInstance()->getRequest()->getHost() .'/' .$url);
    ...
    {
       @file_put_contents($result['path'], base64_decode($result['file']));
    }
    ...
    $result = array('file' => base64_encode(file_get_contents($path)), 'url' => $url, 'path' => $path, 'contentType' => $contentType, 'size' => filesize($path));
  ...
  public function getMedia($slotWidth, $slotHeight, $type = 'c')
  {
       $dimensions = aDimensions::constrain(
        ...
        array(
          "width" =>  $slotWidth,
          "height" => ($this->getHeight() > $slotHeight)?  $slotHeight: false,
          "resizeType" => $type));
       
       return $this->getScaled($dimensions);
  }
  
  public function getMediaQueries()
  {
     $maxHeight = sfConfig::get('app_arquematics_image_height_max');
     $mediaQueryWidthSizes = sfConfig::get('app_arquematics_image_width_sizes');
     
     $ret = array();
     
     foreach ($mediaQueryWidthSizes as $width)
     {
         $ret[] = array_merge($this->getMedia($width,$maxHeight),
                 array('width'  => $width));
     }
