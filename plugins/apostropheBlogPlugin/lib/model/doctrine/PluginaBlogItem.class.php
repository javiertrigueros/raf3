...
abstract class PluginaBlogItem extends BaseaBlogItem
{
  ...
  public function preUpdate($event)
  {
    if($this->update)
    {
      ...
      {
        // If the slug hasn't been altered slugify the title to create the slug.
        $this['slug'] = $this->uniqueSlugFromTitle(aTools::sanitizeText($this->_get('title')));
      }
      else
      {
        // Otherwise slugify the user entered value. (Why is this happening here and not in a form?)
        $this['slug'] = $this->uniqueifySlug(aTools::slugify(aTools::sanitizeText($this['slug'])));
      }
    }
  ...
  public function savePageTags($TagsList, $conn)
  {
      ...
      Doctrine_Query::create($conn)
                    ->from('Tagging')
	            ->addWhere('taggable_id = ?', $pageId)
	            ->addWhere("taggable_model = 'aPage'")
	            ->delete()
                    ->execute();
      ...
      if ($TagsList && (count($TagsList) > 0))
      {
           foreach ($TagsList as $tagId)
           {
               ...
               $tagging->setTaggableModel('aPage');
               $tagging->setTaggableId($pageId);
               $tagging->save($conn);
  ...
  /**
   * actualizar las categorias de la página
   * 
   * @param <array $categoriesList>
   * @param <conn $conn>
   */
  public function savePageCategories($categoriesList, $conn)
  {
      $pageId = $this->Page->getId();
      Doctrine_Query::create($conn)
               ->from('aPageToCategory pc')
               ->where('pc.page_id = ?', $pageId)
               ->delete()
               ->execute();
      
      if ($categoriesList && (count($categoriesList) > 0))
      {
           foreach ($categoriesList as $catid)
           {
               $aPageToCategory = new aPageToCategory();
               $aPageToCategory->setPageId($pageId);
               $aPageToCategory->setCategoryId($catid);
               $aPageToCategory->save($conn);
               $aPageToCategory->free();
               unset($aPageToCategory);
           }
   ...
   */
  public function saveCategories($categoriesList, $conn)
  {

    
     Doctrine_Query::create($conn)
               ->from('aBlogItemToCategory ic')
               ->where('ic.blog_item_id = ?', $this->getId())
               ->delete()
               ->execute();
   ...
   if ($categoriesList && (count($categoriesList) > 0))
   {
       foreach ($categoriesList as $catid)
       {
           ...
           $aBlogItemToCategory->setBlogItemId($this->getId());
           $aBlogItemToCategory->setCategoryId($catid);
           $aBlogItemToCategory->save($conn);
           
           $aBlogItemToCategory->free();
           unset($aBlogItemToCategory);
       }
   }
  }
  ...
  */
  ...
  public function updateRelatedMessages($conn)
  {
      $messages = Doctrine_Core::getTable('arWallMessage')
              ->getMessagesByBlogId($this->getId());
     
      if (count($messages) > 0)
      {
          foreach ($messages as $message)
          {
              $message->setPublishedAt($this->getPublishedAt());
              $message->setIsPublish($this->getIsPublish());
              $message->save($conn);
          }
      }
  }
  ...
  public function getEncryptExcerpt($authUser)
  {
      
     $arWallMessageEnc = Doctrine_Core::getTable('arExcerptEnc')
                        ->retrieveExcerptById($this->getId(), $authUser->getId());
  ...
  public function save(Doctrine_Connection $conn = null)
  {
    ...
    try
    {
        ...
        if (sfConfig::get('app_arquematics_encrypt'))
        {
           ...
           $contentEncrypt = $this->getExcerpt();
           ...
           if ($contentEncryptArr && count($contentEncryptArr))
           {
               foreach ($contentEncryptArr as $keyId => $data)
               {
                  ...
                  $encContent->setABlogItemId($this->getId());
        ...
        $this->updateRelatedMessages($conn);
  ...
  {
      return Doctrine_Core::getTable('Tag')->getQueryById(get_class($this), $this->getId());
  }
  
  public function getTagObjects()
  {
      return Doctrine_Core::getTable('Tag')->getQueryById(get_class($this), $this->getId())->execute();
  }
  ...
  /**
   * 
   * @return <string>: estado del objeto 'draft'|'published'|'schedule';
   */
  public function getStatus()
  {
      if (!$this->getIsPublish()){
        ...
        return ($this->isPassed())?'published':'schedule';
      }
     
  }
  /**
   * 
   * @return <boolean>: true si la fecha de la publicacion esta en el pasado
   */
  public function isPassed()
  {
     $nowTimeStamp = time(); 
     $publishedAtTimeStamp = aDate::normalize($this->getPublishedAt());
     return ($nowTimeStamp >= $publishedAtTimeStamp);
  }
  /**
   * llena los slots del item
   */
  public function populatePage()
  {
      $q = aPageTable::queryWithSlots();
      $q->whereIn('id',array($this->getPageId()));
    
      
      $fast = sfConfig::get('app_a_fasthydrate', false);
      $pageInfo = $q->execute(array(), $fast ? Doctrine::HYDRATE_ARRAY : Doctrine::HYDRATE_RECORD);

      //print_r($pageInfo);
      //print_r($pageInfo[0]['Areas']);
      
      //$page->populateSlotCache($pageInfo['Areas']);
      if (isset($pageInfo[0]))
      {
         $this->Page = aTools::cacheVirtualPage($pageInfo[0]);  
      }
      //el codigo viene de aqui
      //aBlogItemTable::populatePages(array($this));
     
      //$this->Page->populateSlotCache($pageInfo['Areas']);
  }
  /**
   * devuelve el primer objeto image de los slots
   * 
   * @return aMediaItem | boolean false si no tiene nada
   */
  public function getImage()
  {
      $image = $this->getMedia('image', 1);
  ...
  /**
   * comentarios que no tienen parent 
   * 
  ...
  public function getRootComments()
  {
      return Doctrine::getTable('arComment')
              ->getByBlogItemId($this->getId(),  null);
  }
  /**
   * cuenta todos los comentarios de una entrada
   * 
   * @return <int>
   */
  ...
  {
     return Doctrine::getTable('arComment')
              ->countByBlogItemId($this->getId()); 
  }
  
  public function getComments()
  {
     return Doctrine::getTable('arComment')
              -> getAllByBlogItemId($this->getId()); 
  }
}