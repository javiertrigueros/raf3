...
class PluginaBlogItemTable extends Doctrine_Table
{
  ...
  public function addPublished(Doctrine_Query $q)
  {
    ...
    //$q->addWhere($rootAlias.'.is_draft = false AND '. $rootAlias.'.published_at <= NOW()');
    $q->andWhere('is_publish = true AND published_at <= NOW()');
  }
  
  public function addCategoriesForUser(sfGuardUser $user, $admin = false)
  {
    $q = $this->addCategoriesQuery();  
    return Doctrine::getTable('aCategory')->addCategoriesForUser($user, $admin, $q);
  }
  

  public function addCategoriesQuery()
  {
      $q = Doctrine::getTable('aCategory')->createQuery()
        ->addOrderBy('aCategory.name');
  ...
  public function getTagsForCategories($categoryIds, $model, $popular = false, $limit = null)
  {
    ...
    $innerQuery = "SELECT b.id AS id FROM a_blog_item b
                   ...
                   WHERE  b.is_draft = false AND b.published_at < NOW()";
  ...
  public function queryForAuthors()
  {
    ...
    /**
     * blogItems asociados con un mensaje
     * 
     * @param <int $menssageId>
     * @return <aBlogItem aBlogItem>
     */
    public function getMessage($menssageId)
    {
        $q = Doctrine::getTable('aBlogItem')
                ->createQuery('b')
  ...
  public function getTemplateDefaults()
  {
    ...
    */
    ...
    {
        $query = Doctrine_Core::getTable('aBlogItem')
            ->createQuery('u')
        ->where('u.id = ?', $id);
