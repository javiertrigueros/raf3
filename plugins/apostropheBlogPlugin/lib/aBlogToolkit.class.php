...
class aBlogToolkit {
  ...
  static public function searchBody($action, $slugMatch, $modelClass, $categories, sfWebRequest $request)
  {
    if (aTools::$searchService)
    {
      ...
      $query = Doctrine::getTable('aPage')->createQuery('p')
              ->select('p.*, bi.*')
              ->innerJoin('p.BlogItem bi')
              ->where('bi.slug LIKE ?', '%'.$q.'%');
  ...
  static public function filterForEngine($options)
  {
    ...
    if (isset($options['q']) && (strlen($options['q'])))
    {
      ...
      {
        //exit();
        //$query = Doctrine::getTable('aPage')->createQuery('p')->select('p.id')->andWhere("p.slug LIKE '@ar_blog_event_edit'");
        //$query = Doctrine::getTable('aPage')->createQuery('p')->select('p.id')->andWhere('p.slug LIKE ?', $params['slug_pattern']);
        $query = Doctrine::getTable('aPage')->createQuery('p')->select('p.id');
        aTools::$searchService->addSearchToQuery($query, $q);
    ...
    //$q .= 'where ';
    //hack javier segun el tipo de modelo
    $q .= sprintf("where (bi.type = '%s')",($options['modelClass'] == 'aBlogPost')?'post':'event');
    
    unset($options['slug_pattern']);
    ...
    // siempre el orden de publicación
    /* javier
    elseif ($events)
    {
