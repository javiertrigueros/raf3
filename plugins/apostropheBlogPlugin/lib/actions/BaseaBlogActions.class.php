 ...
 */
abstract class BaseaBlogActions extends BaseArActions
{
  ...
  protected function buildQuery($request, $options = array())
  {
    ...
    $q = Doctrine::getTable($this->modelClass)
            ->createQuery()
    ...
    {
      $q->orderBy($this->modelClass.'.published_at DESC');
    }
    
    if ($this->dateRange === 'day')
    {
       $initDate = $this->params['day']['year']."-".$this->params['day']['month']."-".$this->params['day']['day'];
       $endDate = date( "Y-m-d", strtotime( "$initDate +1 day" ) );
      
       $q->andWhere($this->modelClass.'.published_at >= ?', $initDate )
           ->andWhere($this->modelClass.'.published_at <= ?', $endDate);
    }
    else if ($this->dateRange == 'month')
    {
        $initDate = $this->params['day']['year']."-".$this->params['day']['month']."-01";
        $endDate = date( "Y-m-d", strtotime( "$initDate +1 month" ) );
    ...
    {
        $initDate = $this->params['day']['year']."-01-01";
        $endDate = date( "Y-m-d", strtotime( "$initDate +1 year" ) );

        $q->andWhere($this->modelClass.'.published_at >= ?', $initDate )
           ->andWhere($this->modelClass.'.published_at <= ?', $endDate) ;
    }
    
    $q->andWhere($this->modelClass.'.is_publish = true');
  ...
  public function executeShow(sfWebRequest $request)
  {
    ...
    $this->forward404Unless($this->aBlogPost['is_save'] || $this->getUser()->isAuthenticated());
