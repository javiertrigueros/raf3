...
class Base<?php echo ucfirst($this->getModuleName()) ?>GeneratorHelper extends sfModelGeneratorHelper
{
  ...
  {
    return '<li class="a-admin-action-edit"><a title="Edit" class="a-btn icon a-edit no-label" href="'.url_for('@ar_blog_post_edit?page_back=admin&id='.$object->getId()).'"><span class="icon"></span>'.__($params['label'], array(), 'sf_admin').'</a></li>';
 }
  ...
  {
    return '<li class="a-admin-action-list">'.link_to('<span class="icon"></span>'.__($params['label'], array(), 'a_admin'), $this->getUrlForAction('list'), array(), array('class'=>'a-btn icon a-cancel alt')).'</li>';
  }
