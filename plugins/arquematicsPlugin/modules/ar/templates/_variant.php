...
<?php if (count($variants) > 1): ?>
  ...
  <li class="a-ui variant" style="<?php echo $slot->isNew() ? "display:none" : "" ?>" id="a-<?php echo "$pageid-$name-$permid-variant" ?>">
		<a href="#" onclick="return false;" class="a-variant-options-toggle a-btn icon no-label a-settings" id="<?php echo 'a-'.$pageid.'-'.$name.'-'.$permid.'-variant-options-toggle' ?>"><span class="icon"></span><span><?php echo a_('Options'); ?></span></a>
    <ul class="a-ui a-options a-variant-options dropshadow">
      <li class="a-options-heading"><h4><?php echo a_('Options') ?></h4></li>
      <?php foreach ($variants as $variant => $settings): ?>
        ...
        <li id="<?php echo $id ?>-active" class="active current" style="<?php echo $active ? '' : 'display: none' ?>">
          <span class="a-btn alt a-disabled icon a-checked no-bg"><span class="icon"></span><?php echo a_($settings['label']) ?></span>
        </li>
