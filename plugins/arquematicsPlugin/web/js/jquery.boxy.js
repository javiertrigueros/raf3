...
jQuery.extend(Boxy, {
    ...
    askMap: function(question, callback, options) {
        ...
        {
            doc_write = document.write; // Recuerda el metodo original 
        ...
        $('.a-btn').click(function() {
            ...
            Boxy.get(this).hide(function() {
                if (callback) {
                    /*
                    $.each(clicked, function(key, element) {
                        alert('key: ' + key + '\n' + 'value: ' + element);
                    });
                    */
                   if (clicked.id == 'done')
    ...
    ask: function(question, answers, callback, options) {
        ...
        options = jQuery.extend({modal: true, closeable: false},
                                options || {},
                                {show: true, center:true,  unloadOnHide: false});
...
Boxy.prototype = {
	...
	this.getInner().stop().css({width: '', height: ''});
        ...
        this.boxy.find('.close:first').focus();
          ...
          //autofocus en el primer elemento
        if (this.boxy.find('input:visible:enabled:first').length)
        {
            this.boxy.find('input:visible:enabled:first').focus();
        }
        else if (this.boxy.find('textarea:visible:enabled:first').length)
        {
            this.boxy.find('textarea:visible:enabled:first').focus();
        }
        else if (this.boxy.find('select:visible:enabled:first').length)
        {
            this.boxy.find('select:visible:enabled:first').focus();
        }
...
var doc_write
  /**
  * Espera a que esten los scripts de geolocalización cargados
  */
 function waitGeocoder()
 {
      ...
      {
         setTimeout(waitGeocoder, 500); 
      }
      else
      {
            $.getScript('/arquematicsPlugin/js/jquery.ui.addresspicker.js').done(function() { 
            
            var addresspickerMap = $( "#locate_formated_address" ).addresspicker({
		  elements: {
		    map:"#map",
                    lat:"#locate_latitude",
                    lng:"#locate_longitude",    
                    south_west_lat:"#locate_south_west_lat",
                    south_west_lng:"#locate_south_west_lng",
                    north_east_lat:"#locate_north_east_lat",
                    north_east_lng:"#locate_north_east_lng",
                    locality:"#locate_locality",
            ...
            addresspickerMap.addresspicker( "updatePosition");
             ...
             document.write = doc_write; // restaura el metodo
            }).fail(function(jqxhr, settings, exception) {
                console.log(exception); // restaura el metodo
                document.write = doc_write; // restaura el metodo
            });      
      }
 }