...
function smileyMini(image, text) {
	return ' <img class="jm_smiley jm_smiley-' + image + ' jm_images" alt="' + encodeQuotes(text) + '" src="/arquematicsChatPlugin/images/others/blank.gif" /> ';
}
...
function notifyErrorMini() {
	...
	jQuery('#jappix_mini').html(
		'<div class="jm_starter">' + 
			'<a class="jm_pane jm_button jm_images" href="/archat/error" target="_blank" title="' + _e("Click here to solve the error") + '">' + 
...
function displayMessageMini(type, body, xid, nick, hash, time, stamp, message_type) {
	...
	if((last_xid == xid) && (message_type == last_type) && ((stamp - last_stamp) <= 1800))
		...
		if(type == 'groupchat')
			...
			header += '<b class="jm_me" data-xid="' + encodeQuotes(xid) + '">' + __(You) + '</b>';
		else
...
function launchMini(autoconnect, show_pane, domain, user, password) {
	...
	if(MINI_RECONNECT) {
	 ...
	 // Append the Mini stylesheet
           jQuery('head').append('<link rel="stylesheet" href="/arquematicsChatPlugin/css/mini.css" type="text/css" media="all" />');
	
	 // Legacy IE stylesheet
	 if((BrowserDetect.browser == 'Explorer') && (BrowserDetect.version < 7))
		jQuery('head').append('<link rel="stylesheet" href="/arquematicsChatPlugin/css/mini-ie.css" type="text/css" media="all" />');
