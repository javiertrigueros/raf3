<?php
  ...
  $mini = isset($mini) ? $sf_data->getRaw('mini') : null;
  $normal = isset($normal) ? $sf_data->getRaw('normal') : null;
  $big = isset($big) ? $sf_data->getRaw('big') : null;
?>
<?php use_helper('a') ?>
<?php if ($editable): ?>
  ...
  <?php slot("a-slot-controls-$pageid-$name-$permid") ?>
    <?php include_partial('arImageResponsiveSlot/choose', array('action' => 'arImageResponsiveSlot/edit', 'buttonLabel' => a_get_option($options, 'chooseLabel', a_('Choose Image')), 'label' => a_get_option($options, 'browseLabel', a_('Select an Image')), 'class' => 'a-btn icon a-media', 'type' => 'image', 'constraints' => $options['constraints'], 'itemId' => $itemId, 'name' => $name, 'slug' => $slug, 'permid' => $permid, 'now' => false)) ?>
...
<?php if ($item): ?>
    <?php use_stylesheet("/arquematicsExtraSlotsPlugin/css/arquematicsExtraSlots.css"); ?>
    <?php include_component('arMedia','showPicture',array('mediaItem' => $item)); ?> 
<?php endif ?>