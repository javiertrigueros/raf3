<?php
class arBlogSliderSlotComponents extends aSlotComponents
{
  ...
  public function executeNormalView()
  {
    ...
    $this->options['excerptLength'] = $this->getOption('excerptLength', 40);
    ...
    /*
    if (count($this->aBlogPosts) > 0)
    {
        foreach ($this->aBlogPosts as $aBlogPost)
        {
            //$aBlogPost->populatePage();
            if ($aBlogPost->hasMedia('image'))
            {
               $aMediaItem = $aBlogPost->getMedia();
               
               $this->aBlogPostsMedia[] = array(
                   'post' => $aBlogPost,
                   'img' => $aMediaItem->getMedia(
                           $this->options['slideshowOptions']['width'],
                           $this->options['slideshowOptions']['height'],
                           $this->options['slideshowOptions']['resizeType']));
            }
