 ...
 * 
 * javier trigueros martinez de los huertos
 * javiertrigueros@arquematics.com
 */
(function ($) {
    'use strict';
    ...
    function Galereya(options) {
        ...
        var getHeight = function()
        {
            ...
            var $maxCell = $(maxCell);
            var maxCellH = $maxCell.offset().top + $maxCell.height() + 2 * $grid.position().top - $grid.offset().top;
            var maxCellTemH = 0;
            
            $('.galereya-cell').each(function( index ) {
                $maxCell = $(this);
                maxCellTemH = $maxCell.offset().top + $maxCell.height() + 2 * $grid.position().top - $grid.offset().top;
                
                if (maxCellTemH > maxCellH )
                {
                  maxCellH =  maxCellTemH;     
                }
            });
        ...
        var loadImages = function (index, category) {
            ...
            if (index >= $imgs.length) {
                // ha terminado de cargar imagenes
                $grid.trigger({
                    type:"loadImagesEnd",
                    item:"galereya"
                });
        ...
        var placeCell = function (cell, number) {
            ...
            if ((top + $(cell).height()) >= maxCellH)
            {
              maxCellH = top + $(cell).height();
              //ultima celda que se muestra
              maxCell = cell;
            }
        ...
        var createSlide = function (visibleIndex) {
            ...
            $img = $('<img class="galereya-slide-img" src="' + data[index].fullsrc + '" alt="' + data[index].title + '" />').load(function () {
                ...
                $img.click(Handlers.sliderNextClick);
        ...
        var addEventListeners = function () {
            ...
            $grid.bind('loadImagesEnd', Handlers.loadImagesEnd);
